  ...
  methods: {
    ...
    async initRoom() {
      this.try(async () => {
        ...
        const { data: initList } = await db.collection(collection).limit(20).where(this.mergeCommonCriteria({ sendTimeTS: _.gt(new Date().getTime() - (7 * 24 * 60 * 60 * 1000)), })).orderBy('sendTimeTS', 'desc').get()
